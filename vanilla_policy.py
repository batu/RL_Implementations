...
class VanillaPolicyAgent():
    ...
    def __build_network(self, input_dim, output_dim, hidden_dims=[32, 32]):
        ...
        for h_dim in hidden_dims:
            net = tf.keras.layers.Dense(h_dim)(net)
            net = tf.keras.layers.Activation("relu")(net)

        net = tf.keras.layers.Dense(output_dim)(net)
        net = tf.keras.layers.Activation("softmax")(net)

        self.model = tf.keras.Model(inputs=self.X, outputs=net)
    ...
    def __build_train_fn(self):
        ...
        action_onehot_placeholder = tf.keras.backend.placeholder(shape=(None, self.output_dim),
                                                    name="action_onehot")
        discount_reward_placeholder = tf.keras.backend.placeholder(shape=(None,),
                                                    name="discount_reward")

        action_prob = tf.keras.backend.sum(action_prob_placeholder * action_onehot_placeholder, axis=1)
        log_action_prob = tf.keras.backend.log(action_prob)
